...
public class RDMSPlayerController {
	...
	private boolean loadTestData = false;
		...
		try {
			...
			if (psm == null) {
				...
				if (loadTestData) {
					...
					psm.addUpdate(PlayerSimulator.getVariedFakePlayers(500));
				}
			}
			// task health manager...
			if (thm == null) {
	...
	public synchronized List<PlayerState> getPlayers(String entityIdStr,
		...
		for (PlayerState p : psm.getPlayerStateList()) {
			...
			if (entityIdStr != null
				...
				doAdd = false;
				if (debug)
					log.fine("Player filtered because force id "
							+ p.getForceId() + " != " + force.getForceId());
			} else if (type != null
